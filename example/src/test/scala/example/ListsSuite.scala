 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a all zero numbers") {
    assert(sum(List(0,0,0)) === 0)
  }

  test("sum of a few numbers with negatives") {
    assert(sum(List(5,-2,3)) === 6)
  }

  test("max of a few numbers") {
    assert(max(List(2, 1, 4)) === 4)
  }

  test("max of a few negative numbers") {
    assert(max(List(-2, -1, -4)) === -1)
  }

  test("max of a few numbers including a zero") {
    assert(max(List(3, 0, 2)) === 3)
  }

  test("max of a few numbers including a negative") {
    assert(max(List(0, -3, -2)) === 0)
  }
